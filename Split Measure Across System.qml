...
MuseScore {
	...
	onRun: {
		...
		c.rewindToTick(tick)
		var m = c.measure
		//dont split measures if the first point in a measure is selected (the rest of the plugin is allowed to run regardless)
		if (tick != m.firstSegment.tick) {
			cmd('split-measure')
		}
		
		//dont count the newly created 'measure' in the measure count
		c.rewindToTick(tick)
		m = c.measure
		m.irregular = true		
